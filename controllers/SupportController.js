...
const IssueSupportLetterFlowUrl = process.env.issue_support_letter_url;

// Flow secret (if still required by the flow)
const SupportFlowSecret = process.env.support_secret;
const IssueSupportFlowSecret = process.env.issue_support_letter_secret;
...
// issue suport letter
const IssueSupportLetter = async (req, res) => {
  //   const data = req.body; // Expect JSON like: { "CompanyName": "...", ... }
  console.log("📤 sending support request....");
  const {
    azureId,
  ...
  // Add flow_secret to the body (if required by the flow)
  data.flow_secret = IssueSupportFlowSecret;
  ...
  try {
    ...
    // Send request to Power Automate
    const response = await axios.post(IssueSupportLetterFlowUrl, data, {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${accessToken}`,
        // Include x-api-key if the flow still checks it
        "x-api-key": IssueSupportFlowSecret,
      },
    });
